...
{
    Matrix6d J;
    // J.block(0,0,3,3) = Sophus::SO3d::hat(e.so3().log());//e为SE3李群（4X4）通过.so3()提取其中的 李群SO3再通过.log()转为李代数so3,然后hat()转为反对称矩阵
     J.block(0,0,3,3) = Sophus::SO3d::hat(Sophus::SO3d(e.rotationMatrix()).log());
    J.block(0,3,3,3) = Sophus::SO3d::hat(e.translation()); //这里是t，但是书上是p，Sophus::SO3d::hat与Sophus::SE3d::hat不同
...
public:
    ...
    virtual bool read(istream &is) override
    {
        ...
        setEstimate(Sophus::SE3d(Eigen::Quaterniond(data[6],data[3],data[4],data[5]),Eigen::Vector3d(data[0],data[1],data[2])));
    }

    virtual bool write(ostream &os) const override 
    {
        os << id() << " ";
        Eigen::Quaterniond q = _estimate.unit_quaternion();//单位四元数
...
public:
    ...
    virtual bool read(istream &is) override
    {
        ...
        {
            for (size_t j = i; j < information().cols() && is.good(); j++)
            {
                is >> information()(i,j);
                if(i != j) information()(j,i) = information()(i,j);//6X6对角阵
            }
        }
        return true;
    }
...
int main(int argc, char *argv[])
{
    ...
    auto solver = new g2o::OptimizationAlgorithmLevenberg(g2o::make_unique<Block>(g2o::make_unique<LinearSolver>()));
    ...
    while (!fin.eof())
    {
        ...
        if(name == "VERTEX_SE3:QUAT")
        {
            ...
            fin >> index;
            v->setId(index);
            v->read(fin);//这里不用边缘化的原因是两顶点类型相同，不用分块？
    ...
    ofstream fout("./ch10/result_lie.g2o");
