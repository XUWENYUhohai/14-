...
//Map类用于通过C++中普通的连续指针或者数组 （raw C/C++ arrays）来构造Eigen里的Matrix类，这就好比Eigen里的Matrix类的数据和raw C++array 共享了一片地址，也就是引用(模板函数，用于将一串数据映射到一个矩阵或者向量中)
        Eigen::Map<const Eigen::Matrix<double,6,1>> se3(*parameters);	//https://blog.csdn.net/bkhyv/article/details/125379676
        ...
        if(jacobians != NULL) {
            if(jacobians[0] != NULL) {
                ...
                Eigen::Map<Eigen::Matrix<double, 6, 1,Eigen::RowMajor>> J(jacobians[0]);//行优先？
...
void DirectPoseEstimationSingleLayer(
    ...
    options.dynamic_sparsity = true;//一些非线性最小二乘问题具有symbolically稠密，但numerically稀疏。也就是说，在任何给定的状态下，只有少量Jacobian矩阵的项是非零的，但非零的位置和数量根据状态不同而不同。对于这些问题，在分解器迭代中分解稀疏的雅可比矩阵是很有用的，而不是将所有的零项都包含在一个通用分解中。如果您的问题没有这个属性(或者您不知道)，那么最好保不要使用，否则它可能会导致更糟糕的性能。这种设置只会影响到SPARSE_NORMAL_CHOLESKY求解器。
    options.max_num_iterations = 100;
    options.sparse_linear_algebra_library_type = ceres::SUITE_SPARSE;//https://blog.csdn.net/DumpDoctorWang/article/details/84890792//https://blog.csdn.net/DumpDoctorWang/article/details/84890792
    options.minimizer_type = ceres::TRUST_REGION;//https://blog.csdn.net/DumpDoctorWang/article/details/84890792
    options.linear_solver_type = ceres::SPARSE_NORMAL_CHOLESKY;
    options.trust_region_strategy_type = ceres::DOGLEG;//Ceres使用的置信域计算方法
    options.minimizer_progress_to_stdout = false;
    options.dogleg_type = ceres::SUBSPACE_DOGLEG;//https://blog.csdn.net/weixin_40710375/article/details/96497749
