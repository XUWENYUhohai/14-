...
// #include <g2o/core/solver.h>//没用
// #include <g2o/core/sparse_optimizer.h>//没用
...
class EdgeProjectionPoseOnly : public g2o::BaseUnaryEdge<2, Vec2, VertexPose> {
   public:
    ...
    virtual void computeError() override {
        ...
        SE3 T = v->estimate();
        Vec3 pos_pixel = _K * (T * _pos3d);                           //这个为啥不用乘外参cam_ext？,因为这个没有比较左右相机所以不需要？好像左边相机乘了等于没乘
...
class EdgeProjection
   ...
   public:
    ...
    virtual void computeError() override {
        ...
        SE3 T = v0->estimate();
        Vec3 pos_pixel = _K * (_cam_ext * (T * v1->estimate()));//observations_分左右相机
        pos_pixel /= pos_pixel[2];
        _error = _measurement - pos_pixel.head<2>();
    }
    ...
    virtual void linearizeOplus() override {
        ...
        _jacobianOplusXj = _jacobianOplusXi.block<2, 3>(0, 0) *
                           _cam_ext.rotationMatrix() * T.rotationMatrix();//Sophus::Matrix3d == Eigen::Matrix3d
    }
