...
// 32 bit unsigned int, will have 8, 8x32=256,所以需要256*2个点即32x32的图象块
...
int main(int argc, char *argv[])
{
  ...
  cv::imwrite("matcher.png",image_show);//保存图像
  cv::waitKey();
...
// -------------------------------------------------------------------------------------------------- //
// ORB pattern，这是ORB-SLAM2里采用的一种固定的选点模板
...
void ComputeORB(const cv::Mat &img,vector<cv::KeyPoint> &keypoints,vector<DescType> &descriptors)
{
  ...
  for (auto & kp : keypoints)
  {
    ...
    float m01 = 0 , m10 = 0;
    //计算m_pq时x,y是这个区间内的相对x,y
    for (int dx = - half_patch_size; dx < half_patch_size; dx++)//这里不能是size_t，因为其无符号
    {
    ...
    DescType desc(8,0);//Vector<类型>标识符(最大容量,初始所有值)
...
void BfMatch(const vector<DescType> & desc1,const vector<DescType> & desc2,vector<cv::DMatch> &matches)
{
  ...
  for (size_t i1 = 0; i1 < desc1.size(); i1++)
  {
    ...
    //1.queryIdx：当前“匹配点”在查询图像的特征在KeyPoints1向量中的索引号，可以据此找到匹配点在查询图像中的位置
    //2.trainIdx：当前“匹配点”在训练（模板）图像的特征在KeyPoints2向量中的索引号，可以据此找到匹配点在训练图像中的位置
    //3.distance：两个特征点之间的欧氏距离，越小表明匹配度越高
    ...
    for (size_t i2 = 0; i2 < desc2.size(); i2++)
    {
      ...
      for (int k = 0; k < 8; k++)
      {
        distance += _mm_popcnt_u32(desc1[i1][k] ^ desc2[i2][k]);//^异或,_mm_popcnt_u32用于计算32位变量中1的位数
      }
